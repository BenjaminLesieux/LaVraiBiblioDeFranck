...
static int parseLibxml(const char* buf, unsigned int len) 
{
  //ldoc = xmlReadMemory(buf, len, "noname.xml", NULL, 0);
  //if( ldoc == NULL )
  	//return 1;
  if( xmlSAXUserParseMemory(&my_handler, NULL, buf, len) < 0 )
    return 1;
  return 0;
}
...
*/
  ...
  do
  {
    if( parseLibxml(buf, size) )
      return 1;
  } while( --i > 0 );
  t2 = getTicks();
  printf("Libxml (SAX)...: %ld ms\n", t2 - t1);
  releaseLibxml();
#endif
